...
export default function DarkPremiumCollegeCard({ college, streamName = "Engineering", onCompareClick }) {
  ...
  return (
    <motion.div 
      ...
      viewport={{ once: true }}
      whileHover={{ y: -6 }}
      className="bg-white rounded-[2rem] border border-brand-200 shadow-sm hover:border-brand-500 hover:shadow-md transition-all duration-300 group flex flex-col relative overflow-hidden"
    >
      ...
      <div className="p-6 pb-2 flex items-start justify-between relative z-10">
        ...
        <div className="w-16 h-16 rounded-2xl border border-brand-200 bg-white shadow-sm overflow-hidden transform group-hover:scale-105 transition-transform duration-300 flex-shrink-0 p-1">
        ...
        <div className="flex flex-col items-end gap-2">
          ...
          <div className="bg-gradient-premium border border-brand-200 px-2.5 py-1 rounded-lg flex items-center gap-1.5 shadow-sm">
            ...
            <span className="text-[11px] font-bold text-brand-800 font-display">#{rank} Ranked</span>
          </div>
        </div>
      </div>
      ...
      <div className="px-6 pt-2 pb-6 flex-1 flex flex-col relative z-10 text-left">
        ...
        <div className="mb-4">
          <Link to={`/colleges/${college.id}`} className="block group/link">
            <h3 className="text-2xl font-black text-brand-800 leading-tight mb-2 group-hover/link:text-brand-500 transition-colors line-clamp-2 font-display">
              {college.name}
            </h3>
          </Link>
        ...
        ==================================================== */}
        <div className="bg-brand-50 rounded-2xl p-4 flex items-center justify-between border border-brand-200 mb-6">
          <div className="flex flex-col items-center flex-1 border-r border-brand-200 last:border-0">
            <span className="text-[10px] text-brand-800/50 font-bold uppercase tracking-wider mb-1 flex items-center gap-1 font-sans">
               Placements
            </span>
            <span className="text-sm font-black text-brand-800">{placementPercent}</span>
          </div>
          <div className="flex flex-col items-center flex-1 border-r border-brand-200 last:border-0">
            <span className="text-[10px] text-brand-800/50 font-bold uppercase tracking-wider mb-1 flex items-center gap-1 font-sans">
               Avg Pkg
          ...
          <div className="flex flex-col items-center flex-1">
            ...
            <span className="text-sm font-black text-brand-800">{formatFeeShort(college.fees || college.avgFees)}</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 mt-auto pt-4 border-t border-brand-200">
          ...
          <button 
            ...
            className="p-3.5 rounded-xl bg-white hover:bg-brand-50 text-brand-800/60 border border-brand-200 font-bold transition-all shadow-sm hover:border-brand-500 hover:text-brand-500 flex-shrink-0 cursor-pointer"
